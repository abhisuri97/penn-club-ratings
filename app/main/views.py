...
def index():
    ...
    all_c = []
    for c in clubs:
        club_obj = {'id': c.id, 'description': c.description, 'name': c.name, 'categories': c.categories}
        for a in c.answers:
    ...
    return render_template('main/index.html', all_c=all_c, questions=questions, categories=categories)
        ...
        for x in form:
            if x.name.find('_q') > -1:
                if (x.name.find('_q')):
                    ...
                    answer = form['{}_resp'.format(q_id)].data if '{}_resp'.format(q_id) in form else ''
                    rating = x.data
                Answer.newAnswer(answer, rating, current_user.id, q_id, club_id)
         
        flash('Club Review successfully added',
                'form-success')
